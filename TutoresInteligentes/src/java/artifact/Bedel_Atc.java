...
//**************COLOCAR FECHAMENTO DE OLHO APÓS CONFIGURAÇÃO DE GRAFO NO MOODLE *****************************
	...
	void purge_course_cache(String id_curso) throws SQLException, ClassNotFoundException {
		...
		//Para indicar que el agente ha finalizado su trabajo y hay que matarlo
		String string3 = "UPDATE mdl_tutor_nota_perfil SET nro_calculo=0 where nro_calculo=-1";
		System.out.println( "Refrescos necesarios, avisar..." );
		this.conexaoBD("refrescar");
		this.update(string3);

		//Para indicar que el agente ha finalizado su trabajo y hay que matarlo
		String string2 = "INSERT INTO mdl_tutor_perfil VALUES (-1, 'matar', -1, -1)";
		System.out.println( "Finalizando, avisar..." );
		this.conexaoBD("matar jason");
		this.update(string2);
	...
	/*añadido para poder modificar las notas minima y maxima de los perfiles*/
	ArrayList<String> basico = new ArrayList<>();
	ArrayList<String> medio = new ArrayList<>();
	ArrayList<String> avanzado = new ArrayList<>();

	try {
		basico = get_notas_perfil("Básico");
		medio = get_notas_perfil("Médio");
		avanzado = get_notas_perfil("Avançado");

	} catch (SQLException e) {
		System.out.println("Excepcion SQL");
	} catch (ClassNotFoundException e) {
		System.out.println("Excepcion ClassNotFoundException");
	}
	/*aqui acaba lo añadido*/


	//if (nota <= 25){	  //Esta linea se comenta para utitlizar la nota de la base de datos en vez de hacerlo manualmente
	if(nota <= Integer.parseInt(basico.get(0))){
	...
	/*//Este fragmento se comenta y se cambia por el siguiente que obtiene las notas minima y maxima que se introducen al configurar el bloque tutor de la base de datos
	...
	*/
	if (media > Integer.parseInt(basico.get(0)) && media <= Integer.parseInt(basico.get(1))) {
		id_perfil = "1";
	} else if (media > Integer.parseInt(medio.get(0)) && media < Integer.parseInt(medio.get(1))) {
		id_perfil = "2";
	} else if (media >= Integer.parseInt(avanzado.get(0))) {
		id_perfil = "3";
	}
...
/*	Obtiene las notas minimas y maximas de cada uno de los perfiles: Básico, Médio y Avançado*/

	ArrayList<String> get_notas_perfil(String nome_perfil) throws SQLException, ClassNotFoundException {
		//Al introducir la función en Bedel se elimina esta instancia y se cambia el bedel.select del try por this.select
		Bedel_Atc bedel = new Bedel_Atc();
		String string = "SELECT nome, nota_min, nota_max FROM mdl_tutor_perfil where nome='" + nome_perfil + "'";
		ArrayList<String> notas = new ArrayList<>();

		try {
			ResultSet rs = bedel.select(string);
			while (rs.next()) {
				notas.add(rs.getString("nota_min"));
				notas.add(rs.getString("nota_max"));
			}

		} catch (SQLException e) {
			System.out.println("Excepcion SQL");
		} catch (ClassNotFoundException e) {
			System.out.println("Excepcion ClassNotFound");
		}

		if (Integer.parseInt(notas.get(0)) > Integer.parseInt(notas.get(1))) {
			System.out.println("El rango no es valido: " + Integer.parseInt(notas.get(0)) + "~" + Integer.parseInt(notas.get(1)));
			String temp = notas.get(0);
			notas.set(0, notas.get(1));
			notas.set(1, temp);
			System.out.println("Cambio: " + Integer.parseInt(notas.get(0)) + "~" + Integer.parseInt(notas.get(1)));
		}
		return notas;
